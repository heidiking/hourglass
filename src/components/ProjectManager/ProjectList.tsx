      ...
      {(!projects || projects.length === 0) ? (
        ...
        <div className="space-y-2">
          {projects.map(project => (
            <div 
              key={project.id} 
              className="bg-white rounded-lg shadow-sm p-4 border border-gray-200 cursor-pointer hover:bg-gray-50 transition-colors flex justify-between items-center"
            ...
            >
              <div className="flex-1">
                ...
                <div className="flex items-center mt-1 text-sm text-gray-600 space-x-4">
                  <span>
                    { (getProjectTotalTime(project, activities) / (60 * 60 * 1000)).toFixed(1) } hours
                  </span>
                  <span>
                    {formatCurrency(project.earnings || 0)}
                  </span>
                </div>
                <div className="mt-2 flex flex-wrap gap-1">
                  {project.tags && project.tags.slice(0, 3).map((tag, index) => (
                    <span key={index} className={`${tag.color} text-black rounded-full px-2 py-1 text-xs`}>
                      {tag.name}
                    </span>
                  ))}
              ...
              <button 
                type="button"
                className="text-gray-500 hover:text-gray-700 bg-white p-2 rounded ml-2"
                onClick={(e) => {
                  e.stopPropagation();
